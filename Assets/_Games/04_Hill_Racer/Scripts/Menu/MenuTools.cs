...
public class MenuTools : MonoBehaviour {
	...
	void Start () {
        ...
        // unlock all level and cars
        for (int i = 0; i < 7; i++)
		{
			PlayerPrefs.SetInt("Car" + i.ToString(), 3);
			PlayerPrefs.SetInt("Level" + i.ToString(), 3);
			if (i == 0)
				PlayerPrefs.SetInt("myLevel" + i.ToString(), PlayerPrefs.GetInt("myLevel" + i.ToString(), 1));
			else
				PlayerPrefs.SetInt("myLevel" + i.ToString(), PlayerPrefs.GetInt("myLevel" + i.ToString(), 0));
		}
		PlayerPrefs.SetInt("Car7", 3);
		// complete unlock all
