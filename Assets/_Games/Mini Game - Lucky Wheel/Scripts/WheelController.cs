...
namespace FortuneWheel
{
    public class WheelController : MonoBehaviour
    {
        ...
        //[SerializeField] EndController endController;akash
        [SerializeField] GameObject rocketObj;
        private static WheelController _ins;
        public static WheelController ins
        {
            get
            {
                return _ins;
        ...
        public void TurnWheel()
        {
            ...
            if (GameManagerNinjaKnife.Apple >= turnCost) // If player has enough gold to turn the wheel
            {
                ClaimTurnCost();
        ...
        private void ClaimTurnCost()
        {
            ...
            GameManagerNinjaKnife.Apple -= turnCost;
        ...
        private void GiveAwardByAngle()
        {
            ...
            wheelAudio.Stop();
            Debug.Log($"Reward amount: {PiecesOfWheel[rewardIndex].rewardAmount}");
            if (PiecesOfWheel[rewardIndex].rewardAmount != 0)
            {
                T_Clam.text = $"Claim";
                T_Status.text = $"Congratulations!";
                winEffect.SetActive(true);
            }
            else 
            {
                T_Clam.text = $"Back";
                T_Status.text = $"Better luck next time";
            }
            popupPanel.gameObject.SetActive(true);
            MenuController.Instance.onSetState("WheelFinish");
        }
        ...
        [SerializeField] Text T_Clam;
        [SerializeField] Text T_Status;
        public void ClaimReward()
        {
            Debug.Log("Claim reward");
            popupPanel.gameObject.SetActive(false);
            //MenuController.Instance.onSetState(StaticData.SpinWheel);akash
            GameManagerNinjaKnife.Apple += PiecesOfWheel[rewardIndex].rewardAmount;
            
            //endController.resetDiamondStar();akash
            switch (PiecesOfWheel[rewardIndex].rewardCategory)
            {
                case RewardEnum.Zero:
                    {
                        Debug.Log("Zero");

                        confetiEffect.Play();
                        //gameController.GiveCoins(0, false);
                        totalGold += PiecesOfWheel[rewardIndex].rewardAmount * rewardMultiplier;
                    }
                    break;
                case RewardEnum.Hundred:
                    {
                        confetiEffect.Play();
                        //gameController.GiveCoinsIAP(100);
                        totalGold += PiecesOfWheel[rewardIndex].rewardAmount * rewardMultiplier;
                    }
                    break;
                        case RewardEnum.HundredAndFifty:
                    {
                        confetiEffect.Play();
                        //gameController.GiveCoinsIAP(150);
                        totalGold += PiecesOfWheel[rewardIndex].rewardAmount * rewardMultiplier;
                    }
                    break;
                case RewardEnum.ThreeHundred:
                    {
                        confetiEffect.Play();
                        //gameController.GiveCoinsIAP(300);
                        totalEnergy += PiecesOfWheel[rewardIndex].rewardAmount * rewardMultiplier;
                    }
                    break;
                case RewardEnum.TwoHundred:
                    {
                        confetiEffect.Play();
                        //gameController.GiveCoinsIAP(200);
                        totalLife += PiecesOfWheel[rewardIndex].rewardAmount * rewardMultiplier;
                    }
                    break;
                case RewardEnum.TwoHundredFifty:
                    {
                        confetiEffect.Play();
                        //gameController.GiveCoinsIAP(250);
                        totalLife += PiecesOfWheel[rewardIndex].rewardAmount * rewardMultiplier;
                    }
                    break;
                default:
