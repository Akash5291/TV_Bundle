...
public class QRCodeEncodeController : MonoBehaviour {
	...
	public Texture2D GetQRCode(string valueStr)
    {
		Texture2D codeTxt = null;
		...
		codeTxt = new Texture2D(byteMatrix.Width, byteMatrix.Height);

		for (int i = 0; i != codeTxt.width; i++)
		{
			for (int j = 0; j != codeTxt.height; j++)
			{
				if (byteMatrix[i, j])
				{
					codeTxt.SetPixel(i, j, Color.black);
				}
				else
				{
					codeTxt.SetPixel(i, j, Color.white);
				}
			}
		}

		Color32[] pixels = codeTxt.GetPixels32();
		codeTxt.SetPixels32(pixels);
		codeTxt.Apply();
	...
	//	var writer = new QRCodeWriter();
		...
		//Debug.Log("w: " + byteMatrix.Width + ", h: " + byteMatrix.Height);
		m_EncodedTex = new Texture2D(byteMatrix.Width,  byteMatrix.Height);
		//Debug.Log("w: " + m_EncodedTex.width + ", h: " + m_EncodedTex.height);
